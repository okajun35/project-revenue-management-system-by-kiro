...
def test_csv_import():
    ...
    with app.app_context():
        ...
        csv_content = (
            "プロジェクトコード,プロジェクト名,支社名,売上の年度,受注角度,売上（契約金）,経費（トータル）\n"
        ...
        try:
            ...
            for project in imported_projects:
                ...
                print(
                    f"    Branch: {project.branch.branch_name if project.branch else 'N/A'}"
                )
            ...
            print(
                f"✓ Branch created successfully: {test_branch.branch_code} - {test_branch.branch_name}"
            )
        finally:
