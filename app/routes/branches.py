...
def create():
	try:
		...
		if validation_errors:
			...
			return render_template('branches/form.html', branch=branch_data, action='create')
		...
		return render_template('branches/form.html', branch=request.form.to_dict(), action='create')
	except Exception as e:
...
def api_branches():
	try:
		...
		return jsonify({'success': True, 'data': [branch.to_dict() for branch in branches]})
	except Exception as e:
		logger.error(f"支社一覧API エラー: {e}")
		return jsonify({'success': False, 'error': '支社一覧の取得中にエラーが発生しました'}), 500
...
def api_search_branches():
	try:
		...
		return jsonify({'success': False, 'error': '支社検索中にエラーが発生しました'}), 500
...
def api_branches_for_select():
	try:
		...
		return jsonify({'success': False, 'error': '支社選択リストの取得中にエラーが発生しました'}), 500
...
def api_branch_statistics():
	try:
		...
		return jsonify({'success': False, 'error': '支社統計情報の取得中にエラーが発生しました'}), 500
